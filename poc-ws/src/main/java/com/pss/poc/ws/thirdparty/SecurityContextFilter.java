...
public class SecurityContextFilter implements RequestHandler {
	...
	public Response handleRequest(Message message, ClassResourceInfo cri) {
		 ...
		 * if (authValues.size() != 1) { return createFaultResponse(); }
		 * String[] values = authValues.get(0).split(" "); if (values.length !=
		 * 2 || !"Basic".equals(values[0])) { return createFaultResponse(); }
		 * 
		 * String decodedValue = null; try { decodedValue = new
		 * String(Base64Utility.decode(values[1])); } catch (Base64Exception ex)
		 * { return createFaultResponse(); } final String[] namePassword =
		 * decodedValue.split(":"); if (namePassword.length != 2) { return
		 * createFaultResponse(); }
		 */
		/*
		 * String password = users.get(namePassword[0]); if (password == null ||
		 * !password.equals(namePassword[1])) { return createFaultResponse(); }
		 */
