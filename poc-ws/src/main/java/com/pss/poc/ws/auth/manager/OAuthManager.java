...
public class OAuthManager implements AuthorizationCodeDataProvider {
	...
	public List<OAuthPermission> convertScopeToPermissions(Client client, List<String> scopes) {
		...
		/*
		 * for (String scope : scopes) { if (scope.equals(OAuthConstants.
		 * READ_CALENDAR_SCOPE)) { list.add(READ_CALENDAR_PERMISSION); } else if
		 * (scope.startsWith(OAuthConstants. UPDATE_CALENDAR_SCOPE)) { String
		 * description = OAuthConstants.UPDATE_CALENDAR_DESCRIPTION ;
		 * 
		 * String hourValue = scope.substring(OAuthConstants
		 * .UPDATE_CALENDAR_SCOPE.length()); if (hourValue.equals("24")) {
		 * description += " any time of the day"; } else { description +=
		 * hourValue + " o'clock"; } list.add(new OAuthPermission(scope,
		 * description)); }
		 * 
		 * }
		 */
